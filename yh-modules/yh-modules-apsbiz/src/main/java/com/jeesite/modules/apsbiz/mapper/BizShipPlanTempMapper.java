 ...
 */
@Mapper
public interface BizShipPlanTempMapper extends YhBaseMapper<BizShipPlanTemp> {
...
//            "	B.NEED_TRACTOR AS needTractor, " +
            ...
            "FROM ALG_SHIP_PLAN_TEMP A LEFT JOIN ALG_SHIP_FORECAST B ON A.SHIP_FORCAST_ID = B.ID LEFT JOIN ALG_SHIP_INFO C ON B.SHIP_CODE = C.SHIP_CODE " +
            "${ew.customSqlSegment}")
    List<BizShipPlanTemp> queryList(@Param(Constants.WRAPPER) QueryWrapper<BizShipPlanTemp> queryWrapper);
    ...
    @Select(
            "SELECT " +
                    "   COUNT(A.ID) " +
                    "FROM ALG_SHIP_PLAN_TEMP A LEFT JOIN ALG_SHIP_FORECAST B ON A.SHIP_FORCAST_ID = B.ID " +
                    "${ew.customSqlSegment}"
    )
    Long pageCount(@Param(Constants.WRAPPER) QueryWrapper<BizShipPlanTemp> queryWrapper);
...
//            "	B.NEED_TRACTOR AS needTractor, " +
            ...
            "WHERE\n" +
            "\tROW_ID > #{page}")
    List<BizShipPlanTemp> page(@Param(Constants.WRAPPER) QueryWrapper<BizShipPlanTemp> queryWrapper, @Param("page") Integer page, @Param("size") Integer size);
