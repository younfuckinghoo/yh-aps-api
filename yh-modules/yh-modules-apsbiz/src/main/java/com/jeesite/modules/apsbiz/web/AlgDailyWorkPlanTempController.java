...
@RestController
@AllArgsConstructor
@RequestMapping("/bizdailyworkplantemp")
@Tag(name = "bizdailyworkplantemp - (预)作业计划")
public class AlgDailyWorkPlanTempController {

    private final BizDailyWorkPlanTempService bizDailyWorkPlanTempService;
    ...
    /**
     * 分页查询
     * @param bizDailyWorkPlanTemp (预)作业计划
     * @param page 分页-页
     * @param size 分页-数据量
     * @return
     */
    ...
    public R getAlgDailyWorkPlanTempPage(BizDailyWorkPlanTemp bizDailyWorkPlanTemp, Integer page, Integer size) {

        // 补偿计划日期
        if(null == bizDailyWorkPlanTemp.getPlanDate()){
            bizDailyWorkPlanTemp.setPlanDate(dockService.getPlanDate());
        }
        bizDailyWorkPlanTemp.setAlgorithmStateIn(StrUtil.join(",", AlgorithmEnum.STATE12.getStatus(), AlgorithmEnum.STATE13.getStatus()));
        // 查询船舶信息表维护长宽字段
        IPage<BizDailyWorkPlanTemp> pageRes = bizDailyWorkPlanTempService.queryPage(bizDailyWorkPlanTemp, page, size);
    ...
    public R lock(String id) {
        ...
        // 2. 判断ID是存在
        BizDailyWorkPlanTemp bizDailyWorkPlanTemp = bizDailyWorkPlanTempService.infoById(id);
        if(null == bizDailyWorkPlanTemp){
            return R.failed_null("id");
        }
        BizShipRealTime bizShipRealTime = bizShipRealTimeService.infoById(bizDailyWorkPlanTemp.getShipRealTimeId());
    ...
    public R updateById(@RequestBody BizDailyWorkPlanTemp bizDailyWorkPlanTemp) {
        if(StrUtil.isEmpty(bizDailyWorkPlanTemp.getId())){
            return R.failed_empty("id");
        }
        BizDailyWorkPlanTemp base = bizDailyWorkPlanTempService.infoById(bizDailyWorkPlanTemp.getId());
        if(null == base){
            return R.failed_null("id");
        }
        ...
        // 维护不可更改项目
        bizDailyWorkPlanTemp.setShipRealTimeId(base.getShipRealTimeId());

        if(bizDailyWorkPlanTempService.updateOrCleanById(bizDailyWorkPlanTemp)){
            return R.ok();
        }
        return R.failed();
    }
    ...
    public R delById(String id) {
        return bizDailyWorkPlanTempService.deleteById(id);
    }
    ...
    public R submit(String ids) {
        return bizDailyWorkPlanTempService.submit(ids);
    }
    ...
    public R exportSchedulingPlan(BizDailyWorkPlanTemp bizDailyWorkPlanTemp, HttpServletResponse response) {
        if(null == bizDailyWorkPlanTemp.getPlanDate()){
            bizDailyWorkPlanTemp.setPlanDate(dockService.getPlanDate());
        }
        bizDailyWorkPlanTemp.setAlgorithmStateIn(StrUtil.join(",", AlgorithmEnum.STATE12.getStatus(), AlgorithmEnum.STATE13.getStatus()));
        // 查询船舶信息表维护长宽字段
        List<BizDailyWorkPlanTemp> list = bizDailyWorkPlanTempService.queryList(bizDailyWorkPlanTemp);

        if(CollUtil.isNotEmpty(list)){
            // 船舶预报 列表
            BizShipForecast form1 = new BizShipForecast();
            form1.setVoyageNoIn(list.stream().map(BizDailyWorkPlanTemp::getVoyageNo).collect(Collectors.joining(",")));
            ...
            form2.setBerthNameIn(list.stream().map(BizDailyWorkPlanTemp::getBerthNo).collect(Collectors.joining(",")));
            ...
            form3.setShipCodeIn(list.stream().map(BizDailyWorkPlanTemp::getShipCode).collect(Collectors.joining(",")));
            ...
            list.forEach(obj -> {
                ...
                // 船长 船宽
                BizShipInfo bizShipInfo = shipInfoMap.get(bizDailyWorkPlanTemp.getShipCode());
                if(null != bizShipInfo){
                    obj.setShipLength(Convert.toStr(bizShipInfo.getShipLength()));
                    obj.setCabinNum(bizShipInfo.getCabinNum());
                }
        ...
        try(ExcelExport ee = new ExcelExport("智能预排计划", BizDailyWorkPlanTemp.class)){
