...
var isLayui = window.layui && layui.define, $, win, ready = {
  ...
  //btn: [layer.i18n.btnOk, layer.i18n.btnCancel], // Winhai
  ...
  callback([
    ...
    '<div class="'+ doms[0] + (' layui-layer-'+ready.type[config.type]) + (((config.type == 0 || config.type == 2) && !config.shade) ? ' layui-layer-border' : '') + ' ' + (config.skin||'') +'" id="'+ doms[0] + times +'" type="'+ ready.type[config.type] +'" times="'+ times +'" showtime="'+ config.time +'" conType="'+ (conType ? 'object' : 'string') +'" style="z-index: '+ zIndex +'; width:'+ config.area[0] + ';height:' + config.area[1] + ';position:'+ (config.fixed ? 'fixed;' : 'absolute;') +'">'
      ...
      + '<span class="layui-layer-setwin">'+ function(){
        var closebtn = ismax && config.title ? '<a class="layui-layer-min" href="javascript:;"><cite></cite></a><a class="layui-layer-ico layui-layer-max" href="javascript:;"></a>' : ''; // Winhai 必须有标题的清空下才能最小化
...
Class.pt.creat = function(){
  ...
  switch(config.type){
    case 0:
      ...
      config.btn = ('btn' in config) ? config.btn : layer.i18n.btnOk;// Winhai
    ...
    case 2:
      ...
      // 2017-5-13 Winhai 支持post方式提交iframe
      config.content = '<iframe scrolling="'+ (config.content[1]||'auto') +'" allowtransparency="true" id="'
        + doms[4] + times +'" name="'+ doms[4] +''+ times 
  ...
  //config.type == 2 && layer.ie == 6 && that.layero.find('iframe').attr('src', content[0]); // Winhai 不需要ie6
...
Class.pt.offset = function(){
  ...
  that.offsetTop = that.offsetTop > 0 ? that.offsetTop : 0; // 2017-5-8 Winhai Top值不小于0
  ...
  layero.find('.'+ doms[6]).children('a').on('click', function(){
    ...
    if(index === 0){
      if(config.yes){
        ...
        close === false || layer.close(that.index); // 2017-3-31 Winhai 添加返回值如果是false则关闭窗口
      } else if(config['btn1']){
...
//获取当前对话框所在窗口的jQuery对象和Window对象  Winhai
...
//返回当前layer的iframe对象的window对象 Winhai
...
layer.style = function(index, options, limit){
  ...
  btnHeight = layero.find('.'+doms[6]).outerHeight() || 0; // Winhai 兼容 jQuery 3.5
