...
//        }
      ...
      tabitem.tab = tab;
      tabitem.title = title;
      tabitem.refresher = refresher; // 刷新当前标签 Winhai
  ...
  kill : function(position, isNotUpdate){
    ...
    if (isNotUpdate != true){
    	var self = this;
    	// 延迟更新下，否则出现滚动条后，快速关闭右侧导致左侧的菜单未显示 Winhai，https://gitee.com/winhai/jeesite5/issues/IR0LM
    	setTimeout(function(){
    	    //update width
    		self.update();
    	    //to scroll bar 
