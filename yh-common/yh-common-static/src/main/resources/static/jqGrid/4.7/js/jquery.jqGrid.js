 ...
 * Copyright (c) 2008, Tony Tomov, tony@trirand.com, Winhai
 * Dual licensed under the MIT and GPL licenses
 * http://www.opensource.org/licenses/mit-license.php
 * http://www.gnu.org/licenses/gpl-2.0.html
 * https://gitee.com/winhai/jeesite5
 * Date: 2023-09-20
 */
(function ($) {
'use strict';
...
$.extend($.jgrid,{
	...
	scrollsize: function() {
		...
		if (navigator.appName === 'Microsoft Internet Explorer'
			|| (navigator.userAgent.indexOf('Trident/') > 0 && navigator.userAgent.indexOf('rv:11') > 0)
	...
	parseDate : function(format, date, newformat, opts) {
		...
		if(date && date != null) {
			if( !isNaN( date - 0 ) && String(format).toLowerCase() === "u") {
				...
				timestamp = new Date( parseFloat(date) ); // Winhai 修正日期格式化错误问题
	...
	jqID : function(sid){
		...
		return String(sid).replace(/[!"#$%&'()*+,.\/:; <=>?@\[\\\]\^`{|}~]/g,"_"); // Winhai 修改将特殊字符替换为“下划线”
	},
	guid : 1,
	uidPref: 'jqg',
	...
	getAccessor : function(obj, expr) {
		...
		// Winhai 支持 colModel.name 设置 Map，如：optionMap[\'dictType\']
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			if(nm=="actions") {clas = 'actions ' + (clas ? clas : ''); } // Winhai 操作列特有的td属性
			//if(nm=="cb") { clas = 'cb ' + (clas ? clas : ''); } // Winhai 复选框列增加 cb 类，去掉省略号样式（有了，先不需要了）
			...
			result += (clas !== undefined ? (" class=\""+clas+"\"") :"") + ((cm.title && tv) ? (
				// Winhai 当 autoencode 为 false 的时候，如果 title 未 html 编码导致 html 显示不完整
				" title=\""+$.jgrid.stripHtml(!ts.p.autoencode ? $.jgrid.htmlEncode(tv) : tv)+"\"") :"");
...
//		formatter = function (rowId, cellval , colpos, rwdat, _act){
		formatter = function (rowId, cellval , colpos, rwdat, _act, irow){ // Winhai add irow 参数
			var cm = ts.p.colModel[colpos],v;
			// Winhai 格式化formatter前，将当前值存入colModel，方便unformat获取该值
...
//				rowId = String(ts.p.idPrefix) !== "" ? $.jgrid.stripPref(ts.p.idPrefix, rowId) : rowId; // 移动到上面
				...
				var opts= {rowId: rowId, colModel:cm, gid:ts.p.id, pos:colpos, irow:irow }; // Winhai add irow 参数
...
//			v = formatter(rowId,cell,pos,srvr,'add');
			v = formatter(rowId,cell,pos,srvr,'add',irow); // Winhai add irow
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			if(gxml && gl){
				...
				while (j<gl) {
					xmlr = gxml[j];
					rid = getId(xmlr,br+j);
					rid  = ts.p.idPrefix + $.jgrid.jqID(rid);
					...
					j++;
//					if(ir===rn) {break;}
					if(ts.p.datatype === "local" && ir===rn) {break;} // Winhai 支持 jqgrid 树表一级数据分页。
				}
			}
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
					...
					if(idr === undefined || idr === "") {
						...
						if(f.length===0){
							if(dReader.cell){
								...
								idr = ccur != null && ccur[idn] !== undefined && ccur[idn] !== "" ? ccur[idn] : idr;
								ccur=null;
							}
						}
					}
				}
				idr  = ts.p.idPrefix + $.jgrid.jqID(idr);
				altr = rcnt === 1 ? 0 : rcnt;
				cn1 = (altr+i)%2 === 1 ? cn : '';
				if( selected) {
					if( ts.p.multiselect) {
				...
				if(locdata || ts.p.treeGrid===true) {
					...
					rd[ts.p.localReader.id] = rd[locid]; // 本地树表数据，如果 colModel 里没有定义id列，则会报找不到列数据的问题  Winhai
					ts.p.data.push(rd);
...
//				if(ir===rn) { break; }
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
					...
					$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first").append(rowData.join('')); // Winhai 恢复被注释的代码，下面这一行不支持ie8
					//ts.firstElementChild.innerHTML += rowData.join(''); // append to innerHTML of tbody which contains the first row (.jqgfirstrow)
					ts.grid.cols = ts.rows[0].cells; // update cached first row
				}
			}
						...
						if(idr === undefined || idr === "") {
							...
							if(f.length===0){
								if(dReader.cell){
									...
									idr = ccur2 != null && ccur2[idn] !== undefined && ccur2[idn] !== "" ? ccur2[idn] : idr;
									ccur2=null;
								}
							}
						}
...
//			recordsperpage = parseInt(ts.p.rowNum,10),
			recordsperpage = parseInt(ts.p.rowNum == -1 ? 5000 : ts.p.rowNum,10), // 如果设置为 -1 则给一个默认值 5000，否则会显示数据不全 Winhai
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				case "deviceone":
					...
					var dataArr = ts.p.postData, dataStr = '';
					...
					http.on("fail", function(data) {
						if($.isFunction(ts.p.loadError)) { ts.p.loadError.call(ts,data,data,data); }
						if (npage === 1) { endReq(); }
						xhr=null;
					});
					http.request();
		...
		setHeadCheckBox = function ( checked ) {
			...
			var fid = ts.p.frozenColumns ? ts.p.id+"__frozen" : "";
			if(fid) {
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if(ts.p.multiSort) {
				...
				if(ts.p.frozenColumns) {
					...
					$(ts.grid.fhDivRight).find("span.ui-grid-ico-sort").addClass('ui-state-disabled');
					$(ts.grid.fhDivRight).find("th").attr("aria-selected","false");
				}
				...
				if(!ts.p.viewsortcols[0]) {
					if(ts.p.lastsort !== idxcol) {
						if(ts.p.frozenColumns){
							...
							$(ts.grid.fhDivRight).find("span.s-ico").hide();
						}
		...
		if(ts.p.grouping===true) {
			ts.p.scroll = false;
//			ts.p.rownumbers = false;  // Winhai 去掉默认分组情况下不显示行号
		...
		if(this.p.multiselect) {
			...
			this.p.colModel.unshift({name:'cb',width:$.jgrid.cell_width ? ts.p.multiselectWidth+ts.p.cellLayout : ts.p.multiselectWidth,sortable:false,resizable:false,hidedlg:true,search:false,align:'center',fixed:true,classes:'cb'}); // Winhai 复选框列增加 cb 类，去掉省略号样式
		}
...
//			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+">";// Winhai classes
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+" "+(ts.p.colModel[i].classes||'')+"'"+ tooltip+">";// Winhai 表头支持 classes 设定
		...
		imgs = null;
		$(this).append(thead);
//		$(this).append(thead).find("#jqgh_"+ts.p.id+"_rn").width(ts.p.rownumWidth); // 修正多表头情况下，有一点点错位问题 Winhai，暂时无问题，取消设定，如果表头上加内容会不居中
		$("thead tr:first th",this).hover(function(){$(this).addClass('ui-state-hover');},function(){$(this).removeClass('ui-state-hover');});
		if(this.p.multiselect) {
			var emp=[], chk;
			$('#cb_'+$.jgrid.jqID(ts.p.id),this).bind('click',function(){
				ts.p.selarrrow = [];
				var froz = ts.p.frozenColumns === true ? ts.p.id + "__frozen" : "";
				if (this.checked) {
						...
						if (i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled') && !$(this).hasClass("jqfoot")){
								...
								if(froz) {
									...
									// Winhai 右侧冻结列设置
									$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id), ts.grid.fbDivRight )[ts.p.useProp ? 'prop': 'attr']("checked",true);
									$("#"+$.jgrid.jqID(this.id), ts.grid.fbDivRight).addClass("ui-state-highlight");
								}
							}
						}
					});
						...
						if(i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled') && !$(this).hasClass("jqfoot")){
								...
								if(froz) {
									...
									// Winhai 右侧冻结列设置
									$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id), ts.grid.fbDivRight )[ts.p.useProp ? 'prop': 'attr']("checked",false);
									$("#"+$.jgrid.jqID(this.id), ts.grid.fbDivRight).removeClass("ui-state-highlight");
								}
							}
						}
					});
...
//			firstr += "<td role='gridcell' style='height:0px;width:"+w+"px;"+hdcol+"'></td>";
			// Winhai 支持 column 的 classes 设置 width （替换上一行代码）
			...
			if ($(e.target).hasClass('ui-jqgrid-resize-disable')) { return; } // 禁用已经禁用的列拖拽 Winhai
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if($(ptr).length === 0 || ptr[0].className.indexOf( 'ui-state-disabled' ) > -1
					|| ($(td,ts).closest("table.ui-jqgrid-btable").attr('id') || '')
							.replace("__frozen_right","").replace("__frozen","") !== ts.id ) {
				return this;
			}
			...
			if ( !ts.p.multikey ) {
				if(ts.p.multiselect && ts.p.multiboxonly) {
					// 如果点击的是复选框所在单元格时 scb 为 true
					if(scb){
						$(ts).jqGrid("setSelection",ri,true,e);
					} else {
//						// Winhai 取消，点击行的时候选中行，只有点击复选框的是再选中行
//						var frz = ts.p.frozenColumns ? ts.p.id+"__frozen" : "";
...
//								// 右侧冻结列设置
//								$("#"+$.jgrid.jqID(n), "#"+$.jgrid.jqID(frz+'_right')).removeClass("ui-state-highlight");
//								$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(n), "#"+$.jgrid.jqID(frz+'_right'))[ts.p.useProp ? 'prop': 'attr']("checked", false);
//							}
//						});
//						ts.p.selarrrow = [];
//						$(ts).jqGrid("setSelection",ri,true,e);
...
//			.append($('<div style="position:relative;'+(isMSIE && $.jgrid.msiever() < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
			.append(this) // Winhai 注释注释掉上方，因为打开会出现：数据少的情况下，鼠标放到更多按钮上的时候出现横向和竖向滚动条。
			...
			if(!$t.p.multiselect) {
				if(pt.className !== "ui-subgrid") {
					if( $t.p.selrow !== pt.id ) {
						...
						if(fid) {
							...
							// Winhai 右侧冻结列设置
							$("#"+$.jgrid.jqID($t.p.selrow), "#"+$.jgrid.jqID(fid+'_right')).removeClass("ui-state-highlight");
							$("#"+$.jgrid.jqID(selection), "#"+$.jgrid.jqID(fid+'_right')).addClass("ui-state-highlight");
						}
						stat = true;
					} else {
						stat = false;
				...
				if(fid) {
					if(ia === -1) {
						...
						$("#"+$.jgrid.jqID(selection), "#"+$.jgrid.jqID(fid+'_right')).removeClass("ui-state-highlight");
					}
					$("#jqg_"+$.jgrid.jqID($t.p.id)+"_"+$.jgrid.jqID(selection), "#"+$.jgrid.jqID(fid))[$t.p.useProp ? 'prop': 'attr']("checked",stat);
					$("#jqg_"+$.jgrid.jqID($t.p.id)+"_"+$.jgrid.jqID(selection), "#"+$.jgrid.jqID(fid+'_right'))[$t.p.useProp ? 'prop': 'attr']("checked",stat);
				}
				if( onsr ) {
					$($t).triggerHandler("jqGridSelectRow", [pt.id, stat, e]);
					if( $t.p.onSelectRow) { $t.p.onSelectRow.call($t, pt.id , stat, e); }
	...
	resetSelection : function( rowid ){
		return this.each(function(){
			...
			if(rowid !== undefined ) {
				...
				if (fid) { $("#"+$.jgrid.jqID(sr), "#"+$.jgrid.jqID(fid+'_right')).removeClass("ui-state-highlight"); }
				if(t.p.multiselect) {
					$("#jqg_"+$.jgrid.jqID(t.p.id)+"_"+$.jgrid.jqID(sr), "#"+$.jgrid.jqID(t.p.id))[t.p.useProp ? 'prop': 'attr']("checked",false);
					if(fid) { $("#jqg_"+$.jgrid.jqID(t.p.id)+"_"+$.jgrid.jqID(sr), "#"+$.jgrid.jqID(fid))[t.p.useProp ? 'prop': 'attr']("checked",false); }
					if(fid) { $("#jqg_"+$.jgrid.jqID(t.p.id)+"_"+$.jgrid.jqID(sr), "#"+$.jgrid.jqID(fid+'_right'))[t.p.useProp ? 'prop': 'attr']("checked",false); }
					t.setHeadCheckBox( false);
				...
				if(t.p.selrow) {
					...
					if(fid) { $("#"+$.jgrid.jqID(t.p.selrow), "#"+$.jgrid.jqID(fid+'_right')).removeClass("ui-state-highlight"); }
					t.p.selrow = null;
				}
			} else {
		...
		this.each(function() {
			...
			if( $t.p.altRows === true && success ) {
				...
				$($t.rows).each(function(i){
					if(i % 2 === 0) { $(this).addClass(cn); } // Winhai 删除后的斑马线计算 i % 2 === 1  改为  i % 2 === 0
...
//							title = this.title ? {"title":$.jgrid.stripHtml(vl)} : {};
							title = this.title ? {"title":$.jgrid.stripHtml(vl=='&#160;'?'':vl)} : {};// Winhai title显示&#160;修正
							if(t.p.treeGrid===true && nm === t.p.ExpandColumn) {
	...
	addRowData : function(rowid,rdata,pos,src) {
		...
		}catch(e){}// Winhai 加上 try catch 修正IE8下脚本错误。
		...
		if(rdata) {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					id = rowid;
					rowid  = t.p.idPrefix + $.jgrid.jqID(rowid);
					if(ni){
				...
				if( t.p.altRows === true && !aradd) {
					if (pos === "last") {
						if ((t.rows.length-1)%2 === 0)  {$(t.rows[t.rows.length-1]).addClass(cn);} // Winhai 增行后的斑马线计算  (t.rows.length-1)%2 === 1 改为 (t.rows.length-1)%2 === 0
					} else {
						$(t.rows).each(function(i){
...
//						$("tr.footrow td:eq("+i+")",t.grid.fsDiv).html(vl).attr(title); // Winhai 新增底部冻结行的时候，设置底部值。
						// Winhai 多个grid情况下，修正底部统计行错位问题
						$("tr.footrow td:eq("+i+")",$(t.grid.sDiv)).html(vl).attr(title);
						$("tr.footrow td:eq("+i+")",$(t.grid.fsDiv)).html(vl).attr(title); // Winhai 新增底部冻结行的时候，设置底部值。
						$("tr.footrow td:eq("+i+")",$(t.grid.fsDivRight)).html(vl).attr(title); // Winhai 新增底部冻结行的时候，设置底部值。
						success = true;
					}
				} else if(action === "get") {
	...
	showHideCol : function(colname,show,inLabel) { // 新增 inLabel 参数，当列名相同时，使用表头名+表头标题作为判断条件，解决name相同问题 Winhai
		return this.each(function() {
			...
			$(this.p.colModel).each(function(i) {
				...
				// 新增 inLabel 参数，当列名相同时，使用表头名+表头标题作为判断条件，解决name相同问题 Winhai
...
//				$($t).jqGrid("setGridWidth",$t.p.shrinkToFit === true ? $t.p.tblwidth : $t.p.width ); // 不刷新宽度，防止排序后闪眼 Winhai
			}
			if( gh )  {
	...
	hideCol : function (colname,inLabel) {// 新增 inLabel 参数 Winhai
		return this.each(function(){$(this).jqGrid("showHideCol",colname,"none",inLabel);});
	},
	showCol : function(colname,inLabel) {// 新增 inLabel 参数 Winhai
		return this.each(function(){$(this).jqGrid("showHideCol",colname,"",inLabel);});
	},
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			...
			$("#gbox_"+$.jgrid.jqID($t.p.id)).css("width",(nwidth+2)+"px"); // Winhai 修正宽度计算错误
	...
	setGridHeight : function (nh) {
		return this.each(function (){
			...
			if($t.p.frozenColumns === true){
				...
				//$('#'+$.jgrid.jqID($t.p.id)+"__frozen").parent().height(bDiv.height()-16);
				$('#'+$.jgrid.jqID($t.p.id)+"__frozen").parent().height(bDiv.height()-($.jgrid.scrollsize)); // Winhai 修正IE下高度计算错误
				$('#'+$.jgrid.jqID($t.p.id)+"__frozen_right").parent().height(bDiv.height()-($.jgrid.scrollsize)); // Winhai 修正IE下高度计算错误
				// 如果屏幕过小（宽度小于600）或者没有水平滚动条了，则没必要再显示锁定列了 Winhai
				$($t).triggerHandler("jqGridFrozenColumnsToggle");
			}
			$t.p.height = nh;
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		if ( $.fn.jqm && p.jqModal === true) {
			...
			coord.top = p.top+"px";
//			coord.left = p.left;
			coord.left = p.left + $(window).width()/2-p.width/2; // Winhai 调整到屏幕中间
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			case "textarea" :
				...
				if(autowidth) {
					if(!options.cols) { $(elem).css({width:"99.9%"});}
//					if(!options.cols) { $(elem).css({width:"99%",height:"100%"});} // Winhai 兼容firefox
			...
			case "radio":
			case "checkbox":
				// Winhai 新增编辑单元格checkbox支持数组类型（配合选项：items,itemLabel,itemValue）
				if(options.items) {
					elem = document.createElement("div");
					elem.setAttribute("role",eltype);
					$(elem).addClass('icheck');
					var msl, ovm = [];
					if(eltype==="checkbox") {
						msl = true;
					} else { msl = false; }
					...
					if ($.isArray(options.items)){
						...
						for (var i=0; i<options.items.length; i++){
							...
							ov.type = eltype;
							ov.id =  rowid + "_" + options.name + "_" + i;
							ov.name = rowid + "_" + options.name;
							ov.value = value;
							if (options['class']) {
								ov.setAttribute('class', options['class']);
								ov.classList.remove('icheck');
							}
							lbl.appendChild(ov);
							lbl.appendChild(document.createTextNode(" "+(label=='&nbsp;'?'空':label)+" "));
							elem.appendChild(lbl);
							if (!msl &&  ( $.trim(value) === $.trim(vl) || $.trim(label) === $.trim(vl)) ) { ov.checked ="checked"; }
							if (msl && ($.inArray($.trim(label),ovm)>-1 || $.inArray($.trim(value),ovm)>-1)) { ov.checked ="checked"; }
						}
					}
					setAttributes(elem, options, ['items','itemLabel','itemValue']);
				}
			...
			case "select" :
				...
				$(elem).css({width:"99%"}); // Winhai 行编辑控件宽度
				var msl, ovm = [];
				...
				// Winhai 新增编辑单元格select支持数组类型（配合选项：items,itemLabel,itemValue）
					...
					if ($.isArray(options.items)){
						...
						for (var i=0; i<options.items.length; i++){
							...
							if (msl && ($.inArray($.trim(label),ovm)>-1 || $.inArray($.trim(value),ovm)>-1)) { ov.selected ="selected"; }
						}
					}
					setAttributes(elem, options, ['items','itemLabel','itemValue']);
				}
			...
			case "button" :
				...
				if(eltype !== "button"){
					if(autowidth) {
//						if(!options.size) { $(elem).css({width:"99.9%"}); }
						if(!options.size) { $(elem).css({width:"99%"}); } // Winhai 顶满单元格
			...
			case "custom" :
				...
				try {
					...
					else if (e==="e2") { $.jgrid.info_dialog($.jgrid.errors.errcap,"function 'custom_element' "+$.jgrid.edit.msg.novalue,$.jgrid.edit.bClose);}
	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			$.each($t.p.colModel,function(ci){
				...
				thd = $("<div style='position:relative;height:auto;padding-left:0.3em;'></div>"),
				...
				if(this.search){
						...
						if(surl) {
							...
							$.ajax($.extend({
								...
								success: function(res) {
									...
									$("select",self).attr({name:cm.index || cm.name, id: "gs_"+$t.p.idPrefix+$.jgrid.jqID(cm.name)});
					...
					case "text":
						...
						$("td:eq(1)",stbl).append("<input type='text' style='width:100%;' name='"+(cm.index || cm.name)+"' id='gs_"+$t.p.idPrefix+$.jgrid.jqID(cm.name)+"' value='"+df+"'/>");
						$(thd).append(stbl);
						...
						break;
					case "custom":
						$("td:eq(1)",stbl).append("<span style='width:95%;padding:0px;' name='"+(cm.index || cm.name)+"' id='gs_"+$t.p.idPrefix+$.jgrid.jqID(cm.name)+"'/>");
						$(thd).append(stbl);
						try {
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						.addClass("ui-state-default ui-th-column-header ui-th-"+ts.p.direction+" "+(cghi.classes||''))// Winhai classes
//						.css({'height':'22px', 'border-top': '0 none'})
			...
			if (o.useColSpanStyle) {
				...
				$htable.find("span.ui-jqgrid-resize").each(function () {
					...
					if ($parent.is(":visible")) {
						this.style.cssText = 'height: ' + ($parent.height()+2) + 'px !important; cursor: col-resize;';
					}
				});
			...
			$(ts).bind('jqGridResizeStop.setGroupHeaders', function (e, nw, idx) {
				$firstRow.find('th').eq(idx).width(nw-5); // 5个像素不知道哪里来的，解决拖拽表头错位问题
			});
		});
	}
});
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					var ajaxOptions = $.extend({
						...
						complete:function(data,status){
							...
							postdata[idname] = $t.p.idPrefix + $.jgrid.jqID(postdata[idname]);
							...
							complete:function(data,status){
								...
								if(ret[0] === false) {
									...
									if(rp_ge[$t.p.id].reloadAfterSubmit && $t.p.datatype !== "local") {
										...
										if($t.p.treeGrid===true){
												try {$($t).jqGrid("delTreeNode",$t.p.idPrefix+$.jgrid.jqID(postdata[0]));} catch(e){}
										} else {
											for(i=0;i<postdata.length;i++) {
												$($t).jqGrid("delRowData",$t.p.idPrefix+$.jgrid.jqID(postdata[i]));
											}
										}
										$t.p.selrow = null;
										$t.p.selarrrow = [];
...
$.jgrid.extend({
	...
	groupingToggle : function(hid){
		this.each(function(){
			...
			showData,
			collapsed = false,
			frz = $t.p.frozenColumns ? $t.p.id+"__frozen" : false,
			tar2 = frz ? $("#"+$.jgrid.jqID(hid), "#"+$.jgrid.jqID(frz) ) : false,
			tar3 = frz ? $("#"+$.jgrid.jqID(hid), "#"+$.jgrid.jqID(frz)+"_right" ) : false,
			r2 = (tar2 && tar2.length) ? tar2[0].nextSibling : null,
			r3 = (tar3 && tar3.length) ? tar3[0].nextSibling : null;
						...
						if (itemGroupingLevel !== undefined) {
							...
							if (itemGroupingLevel === num + 1) {
								...
								if(frz) {
									...
									$(r3).show().find(">td>span."+"tree-wrap-"+$t.p.direction).removeClass(minus).addClass(plus);
								}
							}
						} else if (showData) {
							$(r).show();
		...
		return this.each(function(){
			...
			$.each(grp.groups,function(i,n){
				...
				if(grp.groupSummaryPos[n.idx] === 'header')  {
					...
					str += "</tr>";
				//} else{
				} else if(grp.groupText != 'none'){ // Winhai 设置groupText为none可隐藏组标题
...
//Editing
	editRow : function(rowid,keys,oneditfunc,successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		o = $.extend(true, {
			keys : true, // 默认注册键盘 回车键 操作 Winhai
			oneditfunc: null,
			successfunc: null,
			url: null,
			extraparam: {},
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
						...
						if(cm[i].editable===true) {
							...
							$(this).css('overflow','visible');// Winhai add td溢出显示，增强 验证，防止验证信息被遮挡
							// Winhai 添加默认参数，如checkbox或select多选情况下，不选择则提交该空值，而不提交null
							if (cm[i].edittype == 'checkbox' || (cm[i].edittype == 'select' && opt.multiple)){
								$(this).prepend('<input type="hidden" name="!'+opt.name+'" value=""/>');
							}
							// ThinGem end
							$.jgrid.bindEv.call($t, elc, opt);
							//Again IE
				...
				if(cnt > 0) {
					...
					if(o.focusField ) {
						...
						setTimeout(function(){
							...
							if(fe.length > 0) {
								// Winhai 新增行，支持select2 获取焦点
								if (fe.eq(0).hasClass('select2-focusser')){
									$(".select2-container:visible:not(:disabled)", ind).select2('focus');
								}else{
									fe.focus();
								}
								// Winhai end
...
//							}
							if (e.keyCode === 13 && !e.shiftKey) {
								...
								// Winhai 编辑行，回车时间修改为获取下一个元素焦点， 修改前
...
//								}
								...
								// Winhai 编辑行，回车时间修改为获取下一个元素焦点， 修改后
								...
								for(var a=0; a < 2; a++){
									...
									// 找到了，则获得焦点，否则是最后一行
									if(nextCol !== false) {
										var cc = $("td:eq("+nextCol+")",$t.rows[iRow]);
										$("input:visible:not(:disabled), select:visible:not(:disabled), textarea:visible:not(:disabled)", cc).focus();
										$(".select2-container:visible:not(:disabled)", cc).select2('focus');
										break; // 跳出循环
									}else{
										// 如果是最后一行，则调用onSelectRow里的新增方法，新增一行
										if ($t.rows.length - 1 == iRow){
											//$($t).jqGrid('addRow', {position: 'last', addRowParams: {keys:true, focusField:true}});
											if( $t.p.editGridAddRow) { $t.p.editGridAddRow.call($t, "new"); }
											else if( $t.p.onSelectRow) { $t.p.onSelectRow.call($t, "new"); }
										}else{
											// Winhai 只点击复选框选中时，则忽略增行后选择列
											if (!$t.p.multiboxonly) {
												$($t).jqGrid('setSelection', $t.rows[++iRow].id, true, event);
											}
											iCol = 0; // 重新开始查找下一个编辑的单元格
										}
									}
								}
								return false;
					...
					// 编辑时，表单控件获取焦点的时候自动选择当前行 Winhai
					if (!$t.p.multiboxonly) {
						$(ind).find('input, select, textarea, .btn').bind("focus", function(){
							if (!($(this).parent().attr('aria-describedby') == $t.p.id + '_cb')){
								$($t).jqGrid('setSelection', rowid, false);
							}
						});
					}
	...
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		if (editable==="1") {
			...
			$('td[role="gridcell"]',ind).each(function(i) {
				...
				$(this).css('overflow','hidden');// Winhai add td溢出显示，增强 验证，防止验证信息被遮挡,,反向方法，取消显示，改为隐藏
			});
			if (cv[0] === false){
				try {
			...
			if(tmp) {
				...
				if (tmp[idname] === undefined || tmp[idname]==="") {
					tmp[idname] = rowid;
				} else if (ind.id !== $t.p.idPrefix + $.jgrid.jqID(tmp[idname])) {
					// rename rowid
					...
					rowid = $t.p.idPrefix + $.jgrid.jqID(tmp[idname]);
					$(ind).attr("id", rowid);
	...
	addRow : function ( p ) {
		p = $.extend(true, {
			...
			initdata : {},
			position : "first",
			src : null, // jqgrid addRow 支持插入到指定位置。  Winhai
			useDefValues : true,
			useFormatter : false,
...
//						p.initdata[this.name] = tmp;
						p.initdata[this.name.replace('[','.').replace(/\'/g,'').replace(']','')] = tmp; // 2018-05-15 修改name为map时，初始值无效问题。 Winhai
					}
				});
			}
//			$($t).jqGrid('addRowData', p.rowID, p.initdata, p.position);
			$($t).jqGrid('addRowData', p.rowID, p.initdata, p.position, p.src); // jqgrid addRow 支持插入到指定位置。 Winhai
			p.rowID = $t.p.idPrefix + $.jgrid.jqID(p.rowID);
				...
				$(ts.rows[i].cells[pos]).bind('click', function() {
					...
					if($(this).hasClass("sgcollapsed")) {
						...
						if(ts.p.subGridOptions.reloadOnExpand === true || ( ts.p.subGridOptions.reloadOnExpand === false && !$(r).hasClass('ui-subgrid') ) ) {
							atd = pos >=1 ? "<td colspan='"+pos+"' class='ui-widget-content subgrid-cell'>&#160;</td>":"";
...
$.jgrid.extend({
	setTreeNode : function(i, len){
		return this.each(function(){
			...
			ldat, lf;
			// Winhai 如果没有子节点，则将该节点转换为子节点图标
			if (i == len){
				var treeclick = $($t.rows[len-1]).find('.treeclick');
				treeclick.removeClass($t.p.treeIcons.plus+" tree-plus");
				treeclick.removeClass($t.p.treeIcons.minus+" tree-minus");
				treeclick.addClass($t.p.treeIcons.leaf+" tree-leaf");
			}
			// Winhai end
			while(i<len) {
				...
				$($t.rows[i].cells[expCol])
					...
					.bind("click",function(e){
						...
						pos = $t.p._index[ind2];
						// Winhai 将无子节点转换为有子节点数据
						if($(this).hasClass('tree-parent')){
							$(this).removeClass('tree-parent');
							if ($(this).hasClass('tree-leaf')){
								$(this).removeClass($t.p.treeIcons.leaf+" tree-leaf").addClass($t.p.treeIcons.minus+" tree-minus");
								$t.p.data[pos][isLeaf] = false;
							}
						}
						// Winhai end
						if(!$t.p.data[pos][isLeaf]){
							if($t.p.data[pos][expanded]){
								...
								// Winhai 展开时重新加载子节点数据
								if ($(this).hasClass('tree-reload')){
									$(this).removeClass('tree-reload');
									var rc = $t.p.data[pos];
									rc[$t.p.treeReader.loaded] = false;
									$.each($($t).jqGrid("getNodeChildren",rc), function(i, d){
										d[$t.p.treeReader.loaded] = false;
										$($t).jqGrid("delTreeNode", d[$t.p.localReader.id]);
									});
								}
								// Winhai end
...
//			$t.p.multiselect = false; // 不限制树表添加复选框 Winhai
			$t.p.rowList = [];
			$t.p.expColInd = 0;
	...
	expandRow: function (record){
		this.each(function(){
			...
			$(childern).each(function(){
				var id  = $t.p.idPrefix + $.jgrid.jqID($.jgrid.getAccessor(this,$t.p.localReader.id));
	...
	expandNode : function(rc) {
		return this.each(function(){
			...
			if($.isFunction(this.p.onExpandNode)) {this.p.onExpandNode(rc);} // Winhai 添加节点展开事件 onExpandNode
		});
	},
	collapseNode : function(rc) {
		return this.each(function(){
			...
			if($.isFunction(this.p.onCollapseNode)) {this.p.onCollapseNode(rc);} // Winhai 添加节点折叠事件 onCollapseNode
		});
	},
...
"use strict";	
	...
	$.fn.fmatter.rowactions = function(act) {
		var $tr = $(this).closest("tr.jqgrow"),
			rid = $tr.attr("id"),
			$id = $(this).closest("table.ui-jqgrid-btable").attr('id')
					.replace(/__frozen([^_]*)$/,'$1').replace(/__frozen_right([^_]*)$/,'$1'),
			$grid = $("#"+$id),
			$t = $grid[0],
			p = $t.p,
...
//			ret = unformatFunc.call(this, val, options, cellval); // Winhai 获取原始数值
