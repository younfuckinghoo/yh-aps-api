	...
	setFrozenColumnsPlus : function (type) {
		return this.each(function() {
			...
			if(maxfrozen>=0 && frozen) {
				...
				$t.grid.fcs[type].fhDiv = $('<div style="position:absolute;'+(isLeft?'left:0':'right:11px')+';top:'+top+'px;height2:'+hth+'px;z-index:1" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>'); // Winhai 修正标题栏高度计算错误
				...
				$t.grid.fcs[type].fbDiv = $('<div style="position:absolute;'+(isLeft?'left:0':'right:11px')+';top:'+(parseInt(top,10)+parseInt(hth,10))+'px;overflow:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>'); // Winhai 修正标题栏高度计算错误
				$("#gview_"+$.jgrid.jqID($t.p.id)).append($t.grid.fcs[type].fhDiv);
				var htbl = $(".ui-jqgrid-htable","#gview_"+$.jgrid.jqID($t.p.id)).clone(true);
				// groupheader support - only if useColSpanstyle is false
//					if($t.p.groupHeader) { // 
				if($t.p.groupHeader || $t.p.groupHeaders) { // Winhai 增加三级表头锁定
				...
				if($t.p.footerrow) {
					...
					$t.grid.fcs[type].fsDiv = $('<div style="position:absolute;left:0px;bottom:0px;" class="frozen-sdiv ui-jqgrid-sdiv"></div>'); // Winhai 修正锁定列，统计行位置计算错误
				...
				$($t).bind('jqGridAfterGridComplete.setFrozenColumns', function () {
					...
					$($t.grid.fcs[type].fhDiv).height($($t.grid.hDiv).height()); // Winhai add 修正锁定列，标题栏高度计算错误
					//$($t.grid.fcs[type].fbDiv).height($($t.grid.bDiv).height()-16);
					$($t.grid.fcs[type].fbDiv).height($($t.grid.bDiv).height()-16+($.jgrid.bigscroll?0:4)); // Winhai 修正IE下高度计算错误
					//$($t.grid.fcs[type].fsDiv).css('top', $($t.grid.hDiv).height() + $($t.grid.fcs[type].fbDiv).height() - $($t.grid.fcs[type].fsDiv).height() - 1 + 5); // Winhai 修正锁定列，统计行位置计算错误
					// Winhai 凑合用IE8下冻结列显示问题
					...
					$($t.grid.bDiv).scroll(); // Winhai 排序后，重置冻结列滚动位置
					if($t.p.hoverrows === true) {
