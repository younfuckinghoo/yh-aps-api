...
//					queryWrapper.apply("DATE_FORMAT("+finalAlias + fieldName+",'%Y-%m-%d') = DATE_FORMAT({0},'%Y-%m-%d')",value);
				}
				...
				if(jhyjField.datetimeEq()){
					queryWrapper.apply("DATE_FORMAT("+finalAlias + fieldName+",'%Y-%m-%d %H:%i:%S') = DATE_FORMAT({0},'%Y-%m-%d %H:%i:%S')",value);
				}
